...
const (
	...
	backupFilePermission   = 0440
	logDirectoryPermission = 0750
...
var (
	logFileSizeThreshold = flag.String("log-file-size",
...
func newFileHook(logFilePath string, logFormat logrus.Formatter) (*FileHook, error) {
	...
	if os.IsNotExist(err) {
		if err := os.MkdirAll(logFileRootDir, logDirectoryPermission); err != nil {
...
// checkNeedsRotation checks to see if a file has grown too large
func (hook *FileHook) checkNeedsRotation() bool {
...
func (f *fileHandler) rotate() error {
	...
	if err := os.Chmod(rotatedLogFileLocation, backupFilePermission); err != nil {
